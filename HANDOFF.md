...
- **Watch out:** ⚠️ **Why this was a factuality problem, not just tone.** The claim asserted a regulatory posture that nothing in `source-private/` establishes — no covered-entity determination, no BAA, no assessment. `AGENTS.md` §3.2 requires every claim to declare an evidence tier, and this one had none while reading as fact; §3.3 says to identify uncertainty rather than guess. The replacement is grounded: §4's never-commit list already names guest, donor, employee, and volunteer records as categories the environment holds. ⚠️ **This entry deliberately does not repeat the removed phrase.** A first pass quoted it verbatim, which left the term present in a public, recruiter-facing file — a "we removed this" framing still puts the words on the page. If a future entry needs to reference it, describe it, don't quote it. ⚠️ **It is not gone from history.** It was public on `feat/service-navigation` from 2026-09-02, and commit `751017c`'s own message quotes it. Only a history rewrite removes those, and the working tree is what a reader actually sees.
...
- **Commit:** `50e872d` · PR branch `chore/handoff-on-default-branch`
